...
import {
  ...
  HardHat, ArrowRight, Sparkles, Shield, MessageCircle, ChevronLeft,
} from 'lucide-react';
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-500/20 to-pink-500/20 rounded-full border border-purple-500/30 mb-6">
            ...
            <span className="text-sm text-purple-300">프리미엄 광고로 우수 인재 확보!</span>
          </div>
          ...
          <p className="text-gray-400 max-w-2xl mx-auto">
            분양현장에 최적화된 광고 상품으로<br />
            경험 많은 분양상담사를 빠르게 모집하세요.
          </p>
                ...
                <div className={`p-6 ${plan.recommended ? 'pt-10' : ''} bg-slate-800/50 flex-1 flex flex-col`}>
                  <div className="flex items-center justify-between mb-4">
                    <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r ${plan.gradientFrom} ${plan.gradientTo}`}>
                      {plan.tier === 'unique' ? <Star className="w-3 h-3 fill-current" /> : plan.tier === 'normal' ? <Zap className="w-3 h-3" /> : <Crown className="w-3 h-3" />}
                      {plan.name}
                    </span>
