...
import type { AgentJobListing, AgentBenefit, QuickApplication } from '@/types';
...
export default function JobDetailPage() {
  ...
  const [relatedTab, setRelatedTab] = useState<'region' | 'type' | 'salary'>('region');
  ...
  const handleBookmark = () => {
    ...
    if (isBookmarked) {
      // 북마크 제거
      const newBookmarks = bookmarks.filter((b: { jobId: string }) => b.jobId !== job.id);
  ...
  const validateForm = (): boolean => {
    ...
    } else if (!/^01[0-9]-?[0-9]{3,4}-?[0-9]{4}$/.test(applyForm.phone.replace(/-/g, ''))) {
      errors.phone = '올바른 연락처를 입력해주세요';
    }

    if (applyForm.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(applyForm.email)) {
      errors.email = '올바른 이메일을 입력해주세요';
    }

    if (!applyForm.agreePrivacy) {
      errors.agreePrivacy = true;
    }
  ...
  const closeApplyModal = () => {
    ...
    setApplyStep('form');
    setApplyForm({ name: '', phone: '', email: '', message: '', agreePrivacy: false });
    setFormErrors({});
  };
                ...
                <div className="relative bg-gray-100 rounded-xl overflow-hidden h-64">
                  ...
                  <div className="absolute inset-0 opacity-10">
                    <div className="absolute inset-0" style={{
                      backgroundImage: `repeating-linear-gradient(0deg, transparent, transparent 20px, #94a3b8 20px, #94a3b8 21px),
                                        repeating-linear-gradient(90deg, transparent, transparent 20px, #94a3b8 20px, #94a3b8 21px)`,
                    }} />
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={(e) => setApplyForm({ ...applyForm, phone: formatPhoneNumber(e.target.value) })}
                  ...
                  <div className={`p-4 rounded-xl ${formErrors.agreePrivacy ? 'bg-red-50' : 'bg-gray-50'}`}>
                    <label className="flex items-start gap-3 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={applyForm.agreePrivacy}
                        onChange={(e) => setApplyForm({ ...applyForm, agreePrivacy: e.target.checked })}
                        className="mt-0.5 w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                      />
                      <span className="text-sm text-gray-600">
                        <span className="text-red-500">[필수]</span> 개인정보 수집 및 이용에 동의합니다.
                        입력하신 정보는 채용 진행을 위해 해당 기업에 전달됩니다.
                      </span>
                    </label>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-3">
                    <div className="flex justify-between py-3 border-b border-gray-100">
                      ...
                      <span className="font-medium text-gray-900">{applyForm.name}</span>
                    </div>
                    <div className="flex justify-between py-3 border-b border-gray-100">
                      <span className="text-gray-500">연락처</span>
                      <span className="font-medium text-gray-900">{applyForm.phone}</span>
                    </div>
                    {applyForm.email && (
                      <div className="flex justify-between py-3 border-b border-gray-100">
                        <span className="text-gray-500">이메일</span>
                        <span className="font-medium text-gray-900">{applyForm.email}</span>
                      </div>
                    )}
                    {applyForm.message && (
                      <div className="py-3 border-b border-gray-100">
                        <span className="text-gray-500 block mb-1">지원 메시지</span>
                        <p className="text-gray-900">{applyForm.message}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-blue-50 rounded-xl p-4">
                    <p className="text-sm text-blue-700">
                      위 정보로 지원하시겠습니까? 지원 후에는 &apos;마이페이지 &gt; 지원내역&apos;에서 확인하실 수 있습니다.
                    </p>
                  </div>
              ...
              {applyStep === 'success' && (
                <div className="text-center py-6">
                  ...
                  <p className="text-gray-600 mb-6">
                    {job.company}에서 곧 연락드릴 예정입니다.<br />
                    지원 내역은 마이페이지에서 확인하실 수 있습니다.
                  </p>
                  <div className="space-y-3">
                    <Link
