...
## 마지막 작업 (2026-09-15) — 부인 STAY 호스트·게스트·안심중개사 흐름 완성

- 제품 방향: 건축물대장 기반 선제 DB 구축은 후순위로 두고, 호스트 직접 등록·게스트 문의로 공급자 DB를 합법적으로 축적한다. 공인중개사 등록은 기존 인증·법정표기 구조를 유지한 별도 안심중개사 경로로 운영한다.
- UI: 지도형 STAY 홈, 상세 날짜 선택·예상 임대료, 호스트 주 단가 등록, 등록 도움 신청, 내 신청, 운영자 접수·위임 초안·소유주 확인·관리자 승인 화면을 연결했다.
- 데이터·보안: 공개 매물 DTO로 연락처·내부 필드를 차단하고, 신규 매물은 모두 검토 후 공개한다. 문의·비공개 읽기 경계·등록 도움 워크플로용 041~043 마이그레이션을 작성했다.
- 인증: 개인/기업 이메일 로그인과 카카오·구글 콜백이 안전한 `/stay` 내부 경로로 복귀하도록 보완했다. 외부 URL과 경로 우회 redirect는 거부한다.
- 실계정 검증: Supabase에 `booin.stay.personal.test@example.com`(seeker), `booin.stay.company.test@example.com`(employer) 테스트 계정을 만들었다. 기업 계정은 사업자·중개사 인증을 부여하지 않은 미인증 상태다. 비밀번호는 저장소와 이 문서에 기록하지 않는다.
- 검증: 전체 Vitest 26파일 306테스트, TypeScript, Next.js 프로덕션 빌드 통과. 로그인 빌드 오류는 로그인 페이지에 Suspense 경계를 추가해 해결했다.
- 배포 순서: 애플리케이션 코드를 먼저 배포한 뒤 운영 Supabase에 041 → 042 → 043 순서로 적용하고, 공개 익명 읽기 차단·인증 사용자 자기 매물 읽기·문의·도움 신청 저장을 라이브에서 확인한다.
