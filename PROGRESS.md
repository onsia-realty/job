...
## 마지막 작업 (2026-06-12) — 마커 네이버페이 부동산 스타일 (집 모양 + 무채색)

### 결론: "눈 아픈" 컬러 마커 → 네이버식 집 모양 무채색 마커 + 탭형 상세 패널

- **집 마커**: 검정(#1E1E23) 지붕(대표면적㎡, 오피스텔 "OP") + 흰 본문(매매 검정 + 전세 초록 #0A8348 병기) + 아래 단지명(흰 halo). 선택 시 본문 다크 반전. DEAL_RAMPS/quintile 색 인코딩 **전부 제거**.
- **이중 집계**: bounds fetch가 trade+rent 병렬 (분양권 탭 +1). `MapComplexPoint`에 `avg_trade_manwon`/`avg_jeonse_manwon`/`rep_area`(primary 면적 중앙값) 추가.
